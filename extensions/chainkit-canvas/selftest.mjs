...
write('01-plan', 'plan.jsonl', [
  ...
  },
  {
    ...
    data: { toolCallId: 'b', success: false },
  },
  {
...
write('06-current-only', 'current-only.jsonl', [
  ...
  {
    type: 'assistant.message',
    data: { model: 'model-current', content: 'current schema only' },
  },
...
eq(
  'a declared completion command reaches the run view',
  s1.declared.completion.run,
  'pnpm check',
);
...
eq(
  'current responseUsage tokens reach the call',
  s5.rounds[0].outputTokens,
  647,
);
eq(
  'mixed current and legacy token schemas are not double-counted',
  s5.outputTokens,
  647,
);
eq(
  'current-only responseUsage tokens reach the call',
  s6.rounds[0].outputTokens,
  806,
);
...
eq(
  'tool status survives',
  s1.rounds[0].steps.filter((x) => x.status === 'failed').length,
  1,
);
eq(
...
eq(
  'a total with an unreported call remains unknown',
...
eq(
  'a stage that wrote nothing reports an empty list, not undefined',
  s1.filesChanged,
  [],
);
...
eq(
  'a cross-stage first call reports the source it inherited',
  s2.resume,
  'plan',
);
eq(
  'cross-stage provenance names the exact source session',
  s2.resumedFrom.sessionId,
  's-plan',
);
eq('a later round on that same session did', s2b.resume, 'plan');
eq('the declared contract is surfaced', Object.keys(s2.expects || {}), [
  'pass',
]);
eq(
...
eq(
  'folding in the record added no process-specific field',
  'verdict' in run,
  false,
);
...
eq(
  'a pending stage has no calls and no spend',
  [pending.rounds.length, pending.aiu],
  [0, 0],
);
eq(
  'a pending stage still shows the model it WILL run',
  pending.model,
  'model-z',
);
eq(
  'a pending stage surfaces its declared inherited session',
  pending.declared.resume,
  true,
);
eq('a stage that ran is not marked pending', run.stages[0].status, 'ran');
eq(
  'the declared config is attached to stages that already ran',
  s2.declared.tools,
  false,
);
// Generic to the end: a manifest naming a stage nobody could have guessed works.
eq(
  'a declared stage name nobody chose in advance survives',
  run.stages[3].model,
  'mystery-9000',
);
  ...
  const put = (dir, model) => {
    ...
    writeFileSync(
      ...
      [
        {
          type: 'assistant.message',
          data: { model, outputTokens: 10, content: '{}' },
        },
...
// way, which says the chain decided against them rather than never got to them.
{
  ...
  writeFileSync(
    ...
    JSON.stringify({
      stages: [{ id: 'cmd' }, { id: 'think' }, { id: 'later' }],
    }),
  );
  ...
  eq(
    'its wall time comes from the record',
    halted.stages.find((s) => s.id === 'cmd').wallMs,
    46,
  );
  eq(
  ...
  // run showed "not started" beside command stages the journal proves had run.
  {
    ...
    eq(
      'the newest journal entry is the stage running now',
      S('gate').status,
      'running',
    );
    eq(
      'a stage the journal has never seen really has not started',
      S('later').status,
      'pending',
    );
  ...
  // by leaving a directory.
  {
    ...
    eq(
      'a fan-out command stage gets a row per element it ran in',
      facts.length,
      1,
    );
    eq(
      'that row is attributed to the element, not element 0',
      facts[0].iter,
      1,
    );
    ...
    eq(
      "a chain's own word for its fan-out unit is carried through",
      f.unit,
      'element',
    );
    writeFileSync(
    ...
    eq(
      'a declared binding name replaces the generic one',
      readRun(fan, r).unit,
      'chunk',
    );
  ...
  // figure for the call that prompted this was 58.83 AiU, minutes later.
  {
    ...
    writeFileSync(
      ...
      [
        {
          type: 'tool.execution_start',
          data: { toolCallId: 'a', toolName: 'view' },
        },
        {
          type: 'tool.execution_complete',
          data: { toolCallId: 'a', success: true },
        },
        {
          type: 'assistant.message',
          data: { model: 'm', outputTokens: 10, content: '…' },
        },
      ]
        .map((l) => JSON.stringify(l))
        .join('\n'),
    ...
    writeFileSync(
      ...
      [
        {
          type: 'assistant.message',
          data: { model: 'm', outputTokens: 10, content: 'ok' },
        },
        { type: 'session.usage_checkpoint', data: { totalNanoAiu: 7e9 } },
        { type: 'result', data: {} },
      ]
  ...
  // cumulative and not two independent calls is `totalPremiumRequests`: 1 then 2.
  {
    ...
    const R = (id, round) =>
      wr.stages.find((s) => s.id === id && (s.round || 0) === round);
    eq(
      'the round that opened a session reports its own cost',
      R('fix', 1).aiu,
      20.74,
    );
    eq(
    ...
    eq(
      'a call with no session id is left exactly as it was',
      R('legacy', 0).aiu,
      9,
    );
    eq(
  ...
  const put = (dir, file) => {
    ...
    writeFileSync(
      ...
      [
        {
          type: 'assistant.message',
          data: { model: 'm', outputTokens: 1, content: '{}' },
        },
        { type: 'result', data: {} },
      ]
        .map((l) => JSON.stringify(l))
...
// A completion failure must be visible before the final run record exists.
{
  ...
  writeFileSync(
    ...
    [
      {
        type: 'assistant.message',
        data: { model: 'm', outputTokens: 1, content: 'done' },
      },
      { type: 'result', data: {} },
    ]
  ...
  eq(
    'generic completion events are exposed by the run reader',
    liveFailure.events.length,
    1,
  );
  eq(
...
// there is no model-call JSONL and the final record does not exist until the run ends.
{
  ...
  eq(
    'a text artifact records its line count',
    text.result.commandResult.artifact.lines,
    2,
  );
  eq(
  ...
  eq(
    'a failed command emits failed status',
    failed.result.commandResult.status,
    'failed',
  );
  eq(
    'a failed command retains its exit code',
    failed.result.commandResult.exitCode,
    7,
  );
  eq(
  ...
  eq(
    ...
    JSON.stringify(broadJson.result.commandResult.artifact.preview).length <=
      10000,
    true,
  );
  eq(
  ...
  const fixtures = [
    ...
    { seq: 4, id: 'failed-command', result: failed.result.commandResult },
    {
      ...
      id: 'foreach-command',
      iter: 1,
      result: text.result.commandResult,
    },
    {
      seq: 6,
      id: 'foreach-command',
      iter: 2,
      result: plain.result.commandResult,
    },
  ];
  writeFileSync(
  ...
  eq(
    'a live successful command is completed',
    stage('text-artifact').status,
    'completed',
  );
  eq(
  ...
  eq(
    'a live failed command is visibly failed',
    stage('failed-command').status,
    'failed',
  );
  eq(
    'foreach command stages remain isolated by iteration',
    live.stages
      .filter((row) => row.id === 'foreach-command')
      .map((row) => row.iter),
    [1, 2],
  );
  eq(
    ...
    live.stages
      .filter((row) => row.commandRuns?.length)
      .map((row) => `${row.id}/${row.iter || 0}`),
    [
      'text-artifact/0',
      'json-artifact/0',
      'plain-output/0',
  ...
  eq(
    'command stages never claim measured model cost',
    stage('text-artifact').aiuKnown,
    false,
  );
  ...
  eq(
    ...
    final.stages
      .filter((row) => row.id === 'loop-command')
      .map((row) => row.round),
    [1, 2],
  );
  ...
  writeFileSync(
    ...
    JSON.stringify({
      stages: [{ id: 'orphan', ord: 1, run: 'sleep', timeoutMs: 1000 }],
    }),
  );
  writeFileSync(
...
// do not exist, and the canvas renders an honest-looking empty state.
{
  ...
  symlinkSync(
    ...
    path.join(
      ...
      'chain-runs',
      'logs',
      'escaped__x__2026-01-03T00-00-00',
    ),
    'dir',
  );
  ...
  eq(
    'union is sorted newest-first across roots',
    both[0].id,
    'engine__b__2026-01-02T00-00-00',
  );
  ...
  eq(
    ...
    listRuns([path.join(rA, 'nope'), rB]).length,
    1,
  );
  eq(
  ...
  eq(
    'readRun refuses a directory outside root/results',
    readRun(outsideRun, rA).stages,
    [],
  );
  readRun(projectRun, rA);
  eq(
    'reading a run does not change its telemetry',
    readFileSync(projectJournal, 'utf8'),
    before,
  );
...
// panel with no error anywhere the operator will look. Parse it here.
{
  ...
  const spendKeys = [...spendMarkup.matchAll(/data-spend-key="([^"]+)"/g)].map(
    (m) => m[1],
  );
  eq(
    'each legend item shares a data-safe key with its matching spend segment',
    spendKeys,
    [
      unusualKey,
  ...
  eq(
    'unusual stage ids do not become selector fragments',
    /^[a-z0-9.-]+$/.test(unusualKey),
    true,
  );
  eq(
  ...
  eq(
    ...
    html.includes(
      '.spend.has-spend-active .split [data-spend-key] { opacity: .3;',
  ...
  class FakeSpendItem {
    ...
    closest(selector) {
      return this.kind === 'legend' &&
        selector === '.legend-item[data-spend-key]'
        ? this
        : null;
    }
  }
  ...
  eq(
    ...
    [
      legendA.classList.contains('spend-active'),
      segmentA.classList.contains('spend-active'),
    ],
    [true, true],
  );
  eq(
    ...
    [
      fakeSpend.classList.contains('has-spend-active'),
      segmentB.classList.contains('spend-active'),
    ],
    [true, false],
  );
  ...
  eq(
    ...
    [
      legendB.classList.contains('spend-active'),
      segmentB.classList.contains('spend-active'),
    ],
    [true, true],
  );
  ...
  eq(
    ...
    ['pointerover', 'pointerout', 'focusin', 'focusout'].every(
      (name) => spendListeners[name],
    ),
    true,
  );
  eq(
  ...
  eq(
    ...
    html.includes('<details class="prompt" data-prompt-key="${esc(key)}"') &&
      html.includes(
        'Prompt sent · ${call.prompt.length.toLocaleString()} characters',
      ) &&
      html.includes(
  ...
  eq(
    'each stage card carries its status as a class',
    html.includes('class="stage s-'),
    true,
  );
  eq(
    'the stylesheet distinguishes the running stage',
    html.includes('.stage.s-running'),
    true,
  );
  eq('a live run is classified separately', runState(null, true), 'live');
  eq(
    'a successful terminal run says delivered',
    runState({ delivered: true }, false),
    'delivered',
  );
  eq(
    'an unverified successful run says completed',
    runState(
      { delivered: false, completed: true, completionStatus: 'absent' },
      false,
    ),
    'completed',
  );
  ...
  eq(
    'the sticky header carries the terminal state',
    html.includes('id="runstate"'),
    true,
  );
  eq(
    'terminal failure changes the whole header',
    html.includes('#top.failed'),
    true,
  );
  eq(
  ...
  eq(
    ...
    runHeadline(
      { completed: true, delivered: false, completionStatus: 'absent' },
      'chunk',
    ),
  ...
  eq(
    ...
    runHeadline(
      {
        delivered: false,
        halted: { stage: 'fix-review', reason: 'repair list reopened F1' },
      },
      'chunk',
    ),
  ...
  eq(
    'the one-line outcome sits beside the badge',
    html.includes('id="runsummary"'),
    true,
  );
  ...
  eq(
    'and it is allowed to shrink enough to do so',
    /#body \{[^}]*min-height: 0/.test(html),
    true,
  );
  eq(
  ...
  eq(
    'it sits with the other stage facts',
    /if \(rsm\) bits\.push\(rsm\)/.test(html),
    true,
  );
  eq(
  ...
  const call = (content, reason) => ({
    ...
    data: {
      ...
      responseChunk: {
        choices: [{ finish_reason: reason, delta: { content } }],
      },
    },
  });
  ...
  writeFileSync(
    ...
    [
      ...
      {
        type: 'assistant.message',
        data: { model: 'm', content: 'the TAIL of it' },
      },
      { type: 'result', data: {} },
    ]
      .map((l) => JSON.stringify(l))
  ...
  eq(
    'the panel shows the beginning, not just the tail',
    cutSay.text.startsWith('BEGINNING'),
    true,
  );
  eq(
    'the panel still shows the tail',
    cutSay.text.includes('the TAIL of it'),
    true,
  );
  ...
  eq(
    'the cut is marked at the seam',
    cutSay.text.includes('cut off here'),
    true,
  );
  ...
  writeFileSync(
    ...
    [
      ...
      {
        type: 'assistant.message',
        data: { model: 'm', content: 'a whole answer, uninterrupted' },
      },
      { type: 'result', data: {} },
    ]
      .map((l) => JSON.stringify(l))
  ...
  eq(
    'an untruncated stage is untouched',
    plainSay.text,
    'a whole answer, uninterrupted',
  );
  ...
  eq(
    'an escaped-newline payload is unescaped for display',
    sayText('a\\nb\\nc'),
    'a\nb\nc',
  );
  ...
  eq(
    ...
    sayHtml(
      'call ' + String.fromCharCode(96) + 'esc()' + String.fromCharCode(96),
    ),
    'call <code>esc()</code>',
  );
  const stepFrom = src.indexOf('const stepHtml =');
  const stepDecl = src.slice(
    stepFrom,
  ...
  eq(
    'the first-row parallel marker stays unchanged',
    adjacentGroups[0].includes('∥2'),
    true,
  );
  ...
  new vm.Script(promptDecl + '\nout = promptMarkdownHtml;').runInContext(
    promptCtx,
  );
  ...
  ok(
    ...
    jsonHtml(
      ['see ' + String.fromCharCode(96) + 'api.ts' + String.fromCharCode(96)],
      0,
    ).includes('<code>api.ts</code>'),
  );
  ok(
  ...
  eq(
    'each pane is guarded independently',
    ((ctx2.out.cards = 'x'), writes.length),
    3,
  );
  ...
  eq(
    'prose that merely mentions an escape is untouched',
    sayText('a\nb\nc\\nd'),
    'a\nb\nc\\nd',
  );
}
...
// parallel.
{
...

  {
    ...
    eq(
      'both are marked with the group size',
      [steps[0].par, steps[1].par],
      [2, 2],
    );
    eq(
...

  {
    ...
    eq(
      'calls that merely follow each other are not parallel',
      annotateConcurrency(steps),
      1,
    );
...

  {
    ...
    eq(
      'a transitive group does not inflate the peak',
      annotateConcurrency(steps),
      2,
    );
...

  {
    ...
    eq(
      'an unfinished call is bounded by the last event',
      annotateConcurrency(steps, t(3)),
      2,
    );
    eq(
      'with no last event it cannot claim overlap',
      annotateConcurrency([tool(t(0), null)]),
      1,
    );
  }

  eq(
    'say steps are ignored',
    annotateConcurrency([{ kind: 'say', text: 'hi' }]),
    0,
  );
